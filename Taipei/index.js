...
function drawLine(lineID, line, c) {
    switch (line["type"]) {
        case "L": {
            drawSimpleLine(lineID, line["pts"], c);
            break;
        }
        case "Q": {
            drawQLine(lineID, line["pts"], c);
            break;
        }
        case "2Q": {
            drawDoubleQLine(lineID, line["pts"], c);
            break;
        }
    }
}

function drawSimpleLine(lineID, args, c) {
...
function drawQLine(lineID, args, c) {
    const fromX = scale(args[0]), fromY = scale(args[1]);
...
function drawDoubleQLine(lineID, args, c) {
    const fromX = scale(args[0]), fromY = scale(args[1]);
